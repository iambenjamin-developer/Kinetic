    ...
    public class PendingMessageProcessorService : BackgroundService
    {
        ...
        private object? DeserializeMessage(string eventType, string payload)
        {
            try
            {
                ...
                {
                    nameof(SharedKernel.Contracts.ProductCreated) => 
                        JsonSerializer.Deserialize<SharedKernel.Contracts.ProductCreated>(payload),
                    nameof(SharedKernel.Contracts.ProductUpdated) => 
                        JsonSerializer.Deserialize<SharedKernel.Contracts.ProductUpdated>(payload),
                    nameof(SharedKernel.Contracts.ProductDeleted) => 
                        JsonSerializer.Deserialize<SharedKernel.Contracts.ProductDeleted>(payload),
                    _ => null
                };
            }
